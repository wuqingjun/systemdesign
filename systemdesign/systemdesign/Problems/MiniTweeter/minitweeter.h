...
using namespace std;
 ...
 public:
	 ...
	 vector<Tweet> getNewsFeed(int user_id) {
		 ...
		 priority_queue < Tweet, vector<Tweet>, tweetgreater > mh;

		 for (auto &t : tweets[user_id])
		 {
			 if (mh.size() >= 10 && t.id > mh.top().id) mh.pop();
			 if (mh.size() < 10) mh.push(t);
		 }
	 ...
	 vector<Tweet>  getTimeline(int user_id) {
		 // Write your code here
		 vector<Tweet> res;
		 for (int i = 0; i < 10 && (int)tweets[user_id].size() - 1 - i >= 0; ++i)
		 {
